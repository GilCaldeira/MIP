<template>
  <div>
    <h2 id="page-heading" data-cy="RentalCarCompanyHeading">
      <span v-text="$t('myappApp.rentalCarCompany.home.title')" id="rental-car-company-heading">Rental Car Companies</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('myappApp.rentalCarCompany.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'RentalCarCompanyCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('myappApp.rentalCarCompany.home.createLabel')"> Create a new Rental Car Company </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && rentalCarCompanies && rentalCarCompanies.length === 0">
      <span v-text="$t('myappApp.rentalCarCompany.home.notFound')">No rentalCarCompanies found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="rentalCarCompanies">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('myappApp.rentalCarCompany.name')">Name</span></th>
            <th scope="row"><span v-text="$t('myappApp.rentalCarCompany.website')">Website</span></th>
            <th scope="row"><span v-text="$t('myappApp.rentalCarCompany.email')">Email</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="myappApp.rentalCarCompany.delete.question"
      ...
      <div class="modal-body">
        <p id="jhi-delete-rentalCarCompany-heading" v-text="$t('myappApp.rentalCarCompany.delete.question', { id: removeId })">
