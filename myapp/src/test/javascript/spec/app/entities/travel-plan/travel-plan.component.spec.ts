...
describe('Component Tests', () => {
  describe('TravelPlan Management Component', () => {
    ...
    it('Should call delete service on confirmDelete', async () => {
      // GIVEN
      travelPlanServiceStub.delete.resolves({});

      // WHEN
      comp.prepareRemove({ id: 123 });
      comp.removeTravelPlan();
      await comp.$nextTick();

      // THEN
      expect(travelPlanServiceStub.delete.called).toBeTruthy();
      expect(travelPlanServiceStub.retrieve.callCount).toEqual(1);
    });
  });
});
